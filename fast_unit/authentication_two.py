...
def start_generating():
    ...
    text_label.config(text="Регенериране")
    frame.create_window(600, 140, window=text_label)
    for _ in range(3):
        text_label.config(text=text_label.cget("text") + ".", font=("Arial", 12))
        text_label.update()
        time.sleep(1)

    text_label.config(text="Комисия номер 1 е разпределена")

    text_label_for_second_teacher.config(text="Регенериране")
    frame.create_window(600, 180, window=text_label_for_second_teacher)
    for _ in range(3):
        text_label_for_second_teacher.config(text=text_label_for_second_teacher.cget("text") + ".", font=("Arial", 12))
        text_label_for_second_teacher.update()
        time.sleep(1)

    text_label_for_second_teacher.config(text="Комисия номер 2 е разпределена")

    text_label_for_third_teacher.config(text="Регенериране")
    frame.create_window(600, 220, window=text_label_for_third_teacher)
    for _ in range(3):
        text_label_for_third_teacher.config(text=text_label_for_third_teacher.cget("text") + ".", font=("Arial", 12))
        text_label_for_third_teacher.update()
        time.sleep(1)

    text_label_for_third_teacher.config(text="Комисия номер 3 е разпределена")

    frame.create_text(
        600,
        260,
        text="Всичко е готово вече можете да създадете файл с бутона по-надолу!",
        font=("Arial", 12),
    )
...
def remove_students():
    ...
    frame.create_window(1085, 100, window=students_to_remove)
...
def remove():
    ...
    for number in found_nums:
        if number in [x[0:5] for x in numbers_for_first_teacher]:
            numbers_for_first_teacher = [num for num in numbers_for_first_teacher if number not in num]

        elif number in [x[0:5] for x in numbers_for_second_teacher]:
            numbers_for_second_teacher = [num for num in numbers_for_second_teacher if number not in num]

        elif number in [x[0:5] for x in numbers_for_third_teacher]:
            numbers_for_third_teacher = [num for num in numbers_for_third_teacher if number not in num]
...
students_to_remove.config(font=("Arial", 16))
text_label = Label(root, text="", font=("Arial", 12), bg="Light Gray")
text_label_for_second_teacher = Label(root, text="", font=("Arial", 12), bg="Light Gray")
text_label_for_third_teacher = Label(root, text="", font=("Arial", 12), bg="Light Gray")